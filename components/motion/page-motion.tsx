...
export function PageMotion() {
  ...
  useGSAP(
    () => {
      ...
      // Уезжающей вглубь панели здесь больше НЕТ (12.09.2026).
      //
      // Было: наклон, масштаб и прозрачность под прокрутку. Замер с
      // торможением ×6 показал, что первый экран — второй по стоимости участок
      // страницы (4118 мс блокировки на 1000 px против 917 мс на «чистом» FAQ),
      // и причина в том, что scrub гонит анимацию по ЖИВОМУ интерфейсу: внутри
      // панели работающее меню, считающиеся числа и доска. Каждый кадр
      // прокрутки заставлял пересобирать этот слой целиком.
      //
      // Эффект стоил больше, чем давал: человек приходит сюда пользоваться
      // системой, а не смотреть, как она кланяется.
      ...
      // ── Свет за секциями ────────────────────────────────────────────────
      // Раньше подсветки разъезжались под прокрутку (`scale` + `opacity` со
      // scrub). Замер 12.09.2026: этот эффект стоил 5,0 с суммарной блокировки
      // за проход страницы — масштабирование огромного радиального градиента
      // перерисовывает полэкрана на каждом кадре. Свет остаётся, он просто
      // больше не анимируется: в статике разницу видит только тот, кто знал,
      // что она была.
    },
    { scope },
  );
