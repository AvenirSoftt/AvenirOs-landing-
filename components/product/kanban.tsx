 ...
 *
 * Колонки прокручиваются ВНУТРИ рамки, а не растягивают страницу — на телефоне
 * это единственный честный способ показать доску: сжать пять колонок в 390 px
 * и оставить их читаемыми нельзя.
 *
 * Доску можно ТЯНУТЬ мышью (`DragScroll`) — ровно как в самой системе. До
 * этого пять колонок на широком экране обрывались справа, и добраться до
 * «Переговоров» с обычной мышью (без горизонтального колеса) было нечем:
 * полосы прокрутки у ленты нет намеренно.
 */
export function KanbanBody({ d }: { d: Dict }) {
  const ui = d.ui;
    ...
    <>
      ...
      <div className="relative">
        ...
        <span
          ...
          aria-hidden="true"
        />
        <DragScroll label={ui.crmTitle} className="flex gap-3 p-3.5 sm:p-4">
          {kanban.map((col, ci) => (
                ...
                {col.leads.map((lead, li) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-md bg-white/[0.08] text-[10px] font-semibold text-snow-2">
...
export function KanbanMockup({ d }: { d: Dict }) {
  return (
    <Chrome title={d.ui.crmTitle} tabs={d.ui.crmTabs}>
      <KanbanBody d={d} />
    </Chrome>
  );
}
