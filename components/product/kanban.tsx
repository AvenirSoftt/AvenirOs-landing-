...
/**
 * CRM-канбан AvenirOS: воронка от «Yangi» до «Muzokara».
 *
...
export function KanbanMockup() {
  return (
    <Chrome title="AvenirOS — CRM · Sotuv voronkasi" tabs={["Kanban", "Ro'yxat", "Analitika"]}>
      <div className="grid grid-cols-2 gap-px bg-line-soft sm:grid-cols-4">
        <Stat label="Jami lidlar" value={String(finance.leads)} sub="Barcha bosqichlar" />
        <Stat label="Voronka summasi" value="596.000.000" sub="Ochiq bitimlar" tone="text-primary-bright" />
        <Stat label="Yutuq ulushi" value="66,67%" sub="Konversiya" tone="text-success" />
        <Stat label="Turib qolgan" value="38" sub="E'tibor talab qiladi" tone="text-warning" />
      </div>
              ...
              {col.leads.map((lead) => (
                ...
                >
                  ...
                  <p className="mt-2 text-[13px] font-semibold text-snow tabular">
                    {lead.amount} <span className="text-[10.5px] font-medium text-snow-2">so&apos;m</span>
                  </p>
